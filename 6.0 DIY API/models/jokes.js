class Jokes {
  static JokesError = class extends Error {
    constructor(message) {
      super(message)
    }
  }
  ...
  constructor() {
    ...
    this.required_patch_fields = Object.freeze(["jokeText", "jokeType"])
  }
  ...
  find(id) {
    return this.jokes.find(joke => joke.id === parseInt(id))
  }

  findIndex(id) {
    return this.jokes.findIndex(joke => Number(joke.id) === Number(id))
  }

  allByType(joke_type) {
    return this.jokes.filter(joke => joke.jokeType.toLocaleLowerCase() === joke_type.toLocaleLowerCase())
  }

  new(post_data) {
    const last_entry = this.last()
    this.jokes.push({id: parseInt(last_entry.id) + 1, jokeText: post_data.text, jokeType: post_data.type})
    return this.last()
  }

  put(id, post_data) {
    const target_index = this.jokes.findIndex(joke => Number(joke.id) === Number(id))
    this.#update_joke(id, post_data, true)
    return this.jokes[target_index]
  }

  update(id, post_data) {
    const target_index = this.findIndex(id)

    this.#update_joke(id, post_data)

    return this.jokes[target_index]
  }

  #update_joke(id, post_data, force_update_all = false) {
    const target_index = this.findIndex(id)

    if (target_index < 0) return;

    if (force_update_all) {
      this.required_patch_fields.forEach(required_field => {
        if (!(required_field in post_data)) {
          throw new Jokes.JokesError('PUT requires field: ' + required_field)
        }
      })
    }

    Object.keys(post_data).forEach(param_key => {
      switch(force_update_all) {
        case true:
          if (param_key in this.jokes[target_index]) {
            this.jokes[target_index][param_key] = post_data[param_key]
          }
          break;
        case false:
          if (param_key in this.jokes[target_index] && post_data[param_key]) {
            this.jokes[target_index][param_key] = post_data[param_key]
          }
          break;
      }
  ...
  delete(id) {
    ...
    if (target_index > 0) {
      this.jokes.splice(target_index,1)
    } else {
      throw new Jokes.JokesError('Joke id not found: ' + id)
    }
  ...
  last() {
    return this.jokes.reduce((last_joke, joke) => Number(joke.id) > Number(last_joke.id) ? joke : last_joke, this.jokes[0])
  }
}
